...
Cloud = function() {
  ...
  var geom1 = new THREE.SphereGeometry(20, 5, 5);
  geom1.translate(-10, 0, 0);
  var geom2 = new THREE.SphereGeometry(20, 5, 5);
  geom2.translate(10, 0, 0);
  var geom3 = new THREE.SphereGeometry(40, 5, 5);
  geom3.translate(0, 0, 0);
  geom.merge(geom1);
  geom.merge(geom2);
  geom.merge(geom3);
  var mat = new THREE.MeshPhongMaterial({
...
Key = function() {
  var geom2 = new THREE.TetrahedronGeometry(10, 4);
...
KeysHolder = function(nKeys) {
  this.mesh = new THREE.Object3D();
  this.keysInUse = [];
  this.keysPool = [];
  for (var i = 0; i < nKeys; i++) {
    var key = new Key();
    this.keysPool.push(key);
  }
};
KeysHolder.prototype.spawnKeys = function() {
  var nKeys = 1;
  var d2 =
    game.seaRadius +
    game.planeDefaultHeight +
  ...
  for (var i = 0; i < nKeys; i++) {
    var key;
    if (this.keysPool.length) {
      key = this.keysPool.pop();
    } else {
      key = new Key();
    }
    ...
    key.angle = -(i * 0.02);
    key.distance = d2 + Math.cos(i * 0.5) * amplitude;
    key.mesh.position.y = -game.seaRadius + Math.sin(key.angle) * key.distance;
    key.mesh.position.x = Math.cos(key.angle) * key.distance;
  }
};
KeysHolder.prototype.rotateKeys = function() {
  for (var i = 0; i < this.keysInUse.length; i++) {
    var key = this.keysInUse[i];
    if (key.exploding) continue;
    key.angle += game.speed * deltaTime * game.keysSpeed;
    if (key.angle > Math.PI * 2) key.angle -= Math.PI * 2;
    key.mesh.position.y = -game.seaRadius + Math.sin(key.angle) * key.distance;
    key.mesh.position.x = Math.cos(key.angle) * key.distance;
    key.mesh.rotation.z += Math.random() * 0.1;
    key.mesh.rotation.y += Math.random() * 0.1;
    var diffPos2 = airplane.mesh.position
      .clone()
      .sub(key.mesh.position.clone());
    var d2 = diffPos2.length();
    if (d2 < game.keyDistanceTolerance) {
      this.keysPool.unshift(this.keysInUse.splice(i, 1)[0]);
      this.mesh.remove(key.mesh);
      particlesHolder.spawnParticles(
        key.mesh.position.clone(),
        5,
        0x009999,
        0.8
      );
...
function createNextLevelEffect() {
  var movementSpeed = 80;
  var totalObjects = 1000;
  var objectSize = 10;
  var sizeRandomness = 4000;
  var colors2 = [0xff0fff, 0x76dc45, 0xfe46a5, 0x458b74, 0xffffff];
  var dirs = [];
  var parts = [];
  function ExplodeAnimation(x, y) {
    ...
    var mat = new THREE.PointsMaterial({
      size: objectSize,
      color: colors2[Math.round(Math.random() * colors2.length)]
    });
    var particles2 = new THREE.Points(geometry, mat);
...
function createKeys() {
  keysHolder = new KeysHolder(20);
  scene.add(keysHolder.mesh);
}
...
function loop() {
  ...
  if (game.status == "playing") {
    if (game.keys >= game.level + 1) {
      ...
      keysToNext.innerHTML = game.level + 1 - game.keys;
    }
    ...
    let tempDistanceMin = game.distanceForLevelUpdate * game.level - 10;
    let tempDistanceMax = game.distanceForLevelUpdate * game.level + 10;
    if (
      Math.floor(game.distance) > tempDistanceMin &&
      Math.floor(game.distance) < tempDistanceMax
    ) {
...
function init(event) {
  ...
  keysToNext = document.getElementById("showKeysToNext");
  resetGame();
  createScene();
  createLights();
  createPlane();
